...
const add = async (req, res) => {
    ...
    try {
        ...
        user.datecreated = new Date().toJSON().slice(0,10).replace(/-/g,'/');
...
const bulkadd = async (req, res) => {
    try {
        ...
        await Promise.all(validData.map(async (data) => {
            // Check if the email exists for the current user
            const existingUserWithEmail = await Contact.findOne({
                email: data.email,
                createBy: userId
            });

            // If no user with the same email exists for the current user, add a new user
        ...
        res.status(200).json({ message: 'Bulk add completed' });
    } catch (err) {
        console.error(err);
...
const deleteMany = async (req, res) => {
    try {
        const contact = await Contact.deleteMany({ _id: { $in: req.body } });
        res.status(200).json({ message: "done", contact })
    } catch (err) {
        res.status(404).json({ message: "error", err })
    }
...
module.exports = { index, add, bulkadd, addPropertyInterest, view, edit, deleteData, deleteMany }